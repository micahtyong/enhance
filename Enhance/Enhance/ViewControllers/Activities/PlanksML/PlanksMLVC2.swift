...
class PlanksMLVC2: CustomTransitionViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    var activity : VisionActivity = VisionActivity(upTo: 30)
    ...
    func runThroughModel(_ jointsImage : UIImage) {
        ...
        guard let model = try? VNCoreMLModel(for: PlanksModelA().model) else { return }
        ...
        let request = VNCoreMLRequest(model: model) { (finishedReq, err) in
            ...
            DispatchQueue.main.async {
                if (firstObservation.identifier == "goodPlanks") {
                    print("Good plank!")
                    self.skeleton.setGood()
                    self.activity.calibrateConfidence(isGood : true)
                } else if (firstObservation.identifier == "badPlanks") {
                    print("Bad plank!")
    ...
    func updateUser(energyPoints : Double) {
        ...
        Enhance.user.addToCore(amount: round(energyPoints * (Double(activity.max) / 10.0)))
        updateDatabase()
    }
    
    func updateDatabase() {
        self.ref.child("Users").child(user.deviceID).setValue(["Name" : Enhance.user.name, "Total" : round(Enhance.user.energyPoints), "Strength" : Enhance.user.strengthTotal, "Stamina" : Enhance.user.staminaTotal, "Core" : Enhance.user.coreTotal])
    }
    ...
    @objc func endML(_ sender:UIButton) {
        ...
        vc.trainingVC.statusBarUpdate(withMessage: "Great form, \(Enhance.user.name)!")
