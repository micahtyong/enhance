...
class SitupMLVC: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
    
    var activity : VisionActivity = VisionActivity(upTo: 20)
        ...
        if modelIsActive() {
            ...
            let request = VNCoreMLRequest(model: model) { (finishedReq, err) in
                ...
                DispatchQueue.main.async {
                    if (firstObservation.identifier == "goodSitups") {
    ...
    func finishingPopup() {
        // Prepare the popup assets
        let title = "SUCCESS"
        let message = "You successfully completed \(activity.max) crunches with a confidence level of \(activity.showConfidence())%.\nYour final score is: \(activity.showConfidence() / 10.0)."
        let image = UIImage(named: "situpDemo3")
        // Create the dialog
        let popup = PopupDialog(title: title, message: message, image: image)
        // Create buttons
